...
function RocketProblem(N=101, tf=10.0;
    ...
    # Initialization
    U0 = [grav for k = 1:N-1]
    initial_controls!(prob)
    rollout!(prob)

    return prob
end

function RocketProblemMPC(prob::Problem, Z_track; kwargs...)
    prob = gen_tracking_problem(prob, N) 
end

function run_Rocket_MPC(prob_mpc, opts_mpc, Z_track, num_iters = length(Z_track) - prob_mpc)
